...
//     this.setState({ name: event.target.value, error: '', users: [] })
//   }
...
//     try {
//       const response = await fetch(`http://localhost:5009/search?name=${name}`)
//       const text = await response.text()
//       const users = JSON.parse(text.replace(/'/g, '"'))
//       if (users.length === 0) {
//         this.setState({ error: 'No users found', users: [] })
//       } else {
//         this.setState({ users, error: '' })
//       }
//     } catch (error) {
//       this.setState({ error: 'Error fetching users', users: [] })
//     }
//   }
...
//         throw new Error('No users found')
//       }
//       this.setState({ users: data, errorMsg: '' })
//     } catch (error) {
...
//   render() {
//     const { searchName, errorMsg, users, redirect } = this.state
